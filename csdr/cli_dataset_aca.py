...
def find_matching_files(store: ObjectStore, pattern: str) -> list[str]:
    """
...
def extract_aca(
    source_location: str = typer.Option(
        ...,
        help="S3/local path containing zip files to extract (e.g. s3://bucket/datasets/aca/0-0-1/raw)"
    ),
...
async def run_index_aca(
    ...
    gpkg_paths = [p for p in gpkg_paths if "reefextent/reefextent.gpkg" in p] # TODO: Why is this line needed? Why not just filter once using find_matching_files?
...
# This command unzips all ACA region zip files from source_location to target_location, then merges all reefextent.gpkg files into a single aca.parquet file at target_location.
